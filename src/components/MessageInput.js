...
export class MessageInput {
    ...
    _render() {
        return `
            <div class="border-t border-lamp-border bg-lamp-card p-4">
                <div class="max-w-4xl mx-auto">
                    <form id="chatForm" class="relative">
                        <div class="bg-lamp-input border border-lamp-border rounded-2xl overflow-hidden focus-within:border-lamp-accent transition-colors">
                            <textarea id="messageInput" 
                                placeholder="Type your message here..." 
                                rows="1"
                                class="w-full bg-transparent px-4 py-3 pr-12 resize-none focus:outline-none"></textarea>
                            
                            <div class="flex items-center justify-between px-3 pb-3">
                                <div class="flex items-center gap-2">
                                    <!-- Model Selector -->
                                    <div class="relative">
                                        <button type="button" id="modelButton" class="flex items-center gap-2 px-3 py-1.5 text-sm bg-lamp-card border border-lamp-border rounded-lg hover:border-lamp-accent transition-colors">
                                            <span id="selectedModelName">GPT-4o</span>
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                                            </svg>
                                        </button>
                                        <div id="modelDropdown" class="hidden absolute bottom-full left-0 mb-2 w-64 bg-lamp-card border border-lamp-border rounded-xl shadow-lg overflow-hidden z-50">
                                    ...
                                    <!-- Web Search Toggle -->
                                    <button type="button" id="webSearchBtn" class="flex items-center gap-1.5 px-3 py-1.5 text-sm border border-lamp-border rounded-lg hover:border-lamp-accent transition-colors">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <!-- Send Button -->
                                <button type="submit" id="sendButton" class="p-2 bg-lamp-accent text-white rounded-lg hover:bg-lamp-hover transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    _updateWebSearchButton() {
        ...
        if (settings?.webSearchEnabled) {
            btn.classList.add('bg-lamp-accent', 'text-white', 'border-lamp-accent');
        } else {
            btn.classList.remove('bg-lamp-accent', 'text-white', 'border-lamp-accent');
        }
    }
